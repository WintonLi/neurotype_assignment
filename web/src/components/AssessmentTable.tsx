...
const AssessmentTable: FunctionComponent<AssessmentTableProps> = ({
  ...
  }, [status, clinicianId, flagged, domain, band, page, pageSize, refreshToken]);

  // Keep the highlighted row valid: default to the first row on load, and
  // whenever the current selection drops out of the (re)fetched page.
  useEffect(() => {
    if (items.length === 0) {
    ...
    const stillPresent = items.some((item) => item.assessment_id === selectedId);
    if (!stillPresent) {
      setSelectedId(items[0].assessment_id);
      onSelect?.(items[0].assessment_id);
    }
